...
# # --- Generic enum column helpers (string enums) ---

# def enum_column(enum: Type[Enum], *, default: Optional[str] = None, nullable: bool = False) -> Mapped[str]:
#     """
#     String-Enum column with inline CHECK (native_enum=False).
#     - Computes VARCHAR length from max enum value length.
#     - Omits constraint name so the CHECK is inline (no duplicate table-level constraints).
#     - Use only with str-based Enums (all of yours like EntityStatusStr, Objective, etc.).
#     """
#     if not issubclass(enum, str):
#         raise TypeError("enum_column expects a `class X(str, Enum)` enum.")
#     max_len = max(len(e.value) for e in enum) if enum else 1
#     return mapped_column(
#         SAEnum(
#             enum,
#             native_enum=False,
...
#             length=max_len,  # ensures e.g. VARCHAR(8) for ACTIVE/INACTIVE/DELETED
#         ),
#         nullable=nullable,
...
# def status_column() -> Mapped[str]:
#     """Convenience for the common ACTIVE/INACTIVE/DELETED status field."""
#     # Ensures: VARCHAR(8) DEFAULT 'ACTIVE' NOT NULL CHECK (status IN (...))
#     from models.enums import EntityStatusStr  # local import to avoid cycles
#     return enum_column(EntityStatusStr, default="ACTIVE", nullable=False)

# # NOTE:
# # If you later need IntEnum storage (e.g., AdDuration), either:
# # 1) use SAEnum(IntEnum, native_enum=False, create_constraint=True) and accept VARCHAR,
# # 2) or write a dedicated `int_enum_column()` that returns Integer + CHECK using a custom TypeDecorator.
