    ...
    camp_payload = registry.CampaignCreate(
        ...
        line_items=[
            registry.LineItemCreate(
                ...
                creatives=[registry.CreativeCreate(asset_url="https://ex.com/a.mp4", mime_type=CreativeMimeType.mp4, duration_seconds=15)],
            )
        ],
    )
...
def test_timestamps_on_all_primary_tables() -> None:
    ...
    camp_payload = registry.CampaignCreate(
        ...
        line_items=[registry.LineItemCreate(name="li", ad_format="STANDARD_VIDEO", bid_cpm=Decimal("10"), pacing_pct=100, targeting={}, creatives=[registry.CreativeCreate(asset_url="https://x/y.mp4", mime_type=CreativeMimeType.mp4, duration_seconds=15)])],
    )
    camp, flight, budget, freq, li, creatives = create_campaign_payload(camp_payload)
    with session_scope() as s:
        s.add(camp)
        ...
        s.add(li)
        s.flush()
        cr = registry.Creative(line_item_id=li.id, asset_url="https://x/y.mp4", mime_type=CreativeMimeType.mp4.value, duration_seconds=15, qa_status="APPROVED")
        s.add(cr)
        s.flush()
